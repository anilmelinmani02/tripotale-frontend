...
export class RegisterComponent {
  ...
  registerWithoutRefcode() {
    ...
    this.firestore.doc('referralCodes/refDoc').set({ [this.userId]: uniqueString }, { merge: true })
      .then(() => {
    ...
    this.firestore.collection('users').doc(this.userId).collection('referrals').doc('referralDoc').set(objToAdd).then(docRef => {
  ...
  // fetch all refCodes
  // fetchValues() {
  //   this.firestore.doc('referralCodes/refDoc').get()
  //     .subscribe((doc) => {
  //       if (doc.exists) {
  //         const data = doc.data() as { [key: string]: string };

  //         console.log("all refferal codes", Object.values(data));
  //       } else {
  ...
  registerUserWithRefCode(refCode: string) {
    ...
    this.itineraryService.getUserId(refCode).subscribe(
      response => {
        ...
        const referralDocRef = this.firestore.collection('users').doc(this.referrerId).collection('referrals').doc('referralDoc');
